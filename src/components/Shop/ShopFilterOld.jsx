...
const ShopFilterOld = ({ getFilters }) => {
  ...
  const toggleClickedLetterHandler = (letter) => {
    dispatch({
      type: "LETTERS",
      array: filters.letters.includes(letter)
  ...
  const ShopFilterByLetters = (
    <ul className="flex flex-wrap items-center gap-2 pt-2">
      {filtersOptions.letters.split("").map((letter, i) => (
        <li
          key={i}
          className={`px-3 py-1 duration-200 bg-gray-200 rounded-full cursor-pointer hover:text-white hover:bg-primary-500 ${
            filters.letters.includes(letter) && "text-white bg-primary-500"
          }`}
        >
          <button onClick={() => toggleClickedLetterHandler(letter)}>
            {letter}
          </button>
        </li>
      ))}
